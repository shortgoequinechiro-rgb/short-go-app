...
export default function InvoicesPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSuccessMsg(`Invoice ${deleteTarget.invoice_number} deleted`)
      setTimeout(() => setSuccessMsg(null), 4000)
                  ...
                  <div className="md:col-span-2 flex flex-wrap gap-1">
                    ...
                    {invoice.status !== 'paid' && (
                      <button
                        ...
                        className="px-2 py-1 text-xs font-medium text-red-600 hover:text-red-700 border border-red-200 rounded-lg hover:bg-red-50"
                      >
                        Delete
                      </button>
                    )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="rounded-3xl bg-white p-6 shadow-lg max-w-sm w-full">
            ...
            <p className="text-slate-600 mb-2">
              Delete invoice <span className="font-semibold">{deleteTarget.invoice_number}</span> for {deleteTarget.owner_name}?
            </p>
